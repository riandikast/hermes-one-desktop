...
[[src/renderer/src/screens/Chat/hooks/useChatScroll.ts#useChatScroll]] exposes `jumpToPresent`, an instant `scrollTop = scrollHeight` snap that re-settles over the next frames + short timeouts (1 frame / 2 frames / 80ms / 250ms). Every scroll-into-the-present path goes through it: first mount, when the user sends a message, and when the tab becomes active. A smooth `scrollIntoView` was used for streaming/auto-scroll but consistently landed short — a smooth animation targets a fixed endpoint that the streaming tail outpaces, and `content-visibility: auto` rows re-measure as they scroll into view, growing `scrollHeight` after the animation completes. Once short, the manual-scroll listener saw `atBottom=false`, marked `userScrolledUp=true`, and subsequent deltas stopped trying — leaving the view a little above the present. Streaming auto-scroll now uses the same instant snap (with its own 1-frame / 120ms settle retry whose effect cleanup cancels the pending retry when the next delta arrives, so only the last delta in a burst runs its retrial), and only fires while the user is pinned to the bottom (a wheel/touch scroll up past 60px pauses it). For user-sent messages the snap is forced regardless of pinned state.
