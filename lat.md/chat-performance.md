...
## Stable streaming (official final-merge contract)

The answer streams LIVE into the pending bubble and is replaced by one authoritative final at completion — the official desktop's stability model, adopted in the dashboard adapter.

[[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#applyDashboardStreamEvent]] renders `message.delta` into the pending assistant bubble (`appendAssistantDelta`). At `message.complete`, [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#mergeFinalAssistantText]] applies the contract: when the final text COVERS the turn's streamed text (normalized includes), every streamed text bubble is discarded and ONE authoritative final bubble is appended (reasoning fully covered by the final is dropped too) — the final always wins, so dropped/garbled delta chunks can never corrupt the answer. When the final is last-turn-only (our backend can omit pre-tool-call text, #746), the streamed bubble is kept and merged via the legacy `mergeStreamedWithFinal` reconciliation. An empty final leaves the streamed bubble pending — `session.info running:false` settles it via [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#finalizeInterruptedMessages]] (un-pends non-empty bubbles, drops empty placeholders). The answer-gate machinery (`useReasoningGate`/`reasoningStall`) was deleted: the live stream + final merge replaced it, and the stall watchdog + quiet-finalize remain as fallbacks.

The merge scope is the CURRENT TURN by POSITION (rows after the last user bubble), not by `turnId` presence: bubbles loaded from state.db (reopened/resumed sessions) carry no `turnId`, and scoping by "no turnId matches everything" would concatenate EVERY answer in the session into the last bubble ("every answer merged / re-answered").

Reasoning rows typewriter-animate only while they are the ACTIVE trailing row ([[src/renderer/src/screens/Chat/HistoryRow.tsx#ReasoningRow]]): a `wasActiveRef` effect snaps `typing` off the instant the row stops being trailing (the answer arrived / the turn ended), so a thought never keeps typing after the response is complete. A late delta re-arms the typewriter via the growth effect, so the snap cannot suppress real incoming text; the 5s no-growth timer remains as a fallback.

### Empty-final completion recovery

The stream is LOSSY (dropped delta chunks leave the pending bubble partial). When `message.complete` arrives with no usable final text (empty/whitespace on every key — the backend can ship `raw=""` for interrupted sentinels, verify-on-stop commentary, or empty summaries), the transport DEFERS the settle: `activeTurnRef` stays set and the quiet-finalize timer stays armed, so the DB reconcile pulls the authoritative full answer from state.db instead of showing the truncated stream until reopen. [[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts]] only defers when a pending streamed bubble actually has content (a genuinely empty turn settles immediately). On reconcile, [[src/renderer/src/screens/Chat/sessionHistory.ts#reconcileAfterDbRefresh]] drops a pending streamed agent bubble when a DB bubble's normalized content fully covers it (`isCoveredByDbBubble`) — the partial is replaced, never stacked next to the full text.
