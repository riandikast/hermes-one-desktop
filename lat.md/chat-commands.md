...
### Delta chunks merge into the last same-kind row

Answer and thinking deltas merge into the last same-kind row of the current turn instead of appending a fresh row per chunk, so the row keeps its React identity and its entry animation plays once per new row rather than per chunk.

Appending a fresh row per chunk remounts the element and replays the `messageIn` entry animation on `.chat-message` (a visible blink per chunk), and interleaved thinking after a tool call would stack the answer below the thinking. The dashboard adapter merges `message.delta` into the last assistant bubble and `thinking.delta`/`reasoning.delta` into the last reasoning row via [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#applyDashboardStreamEvent]]; the legacy IPC path merges `chat-chunk` deltas into the last bubble of the turn ([[src/renderer/src/screens/Chat/hooks/useChatIPC.ts#useChatIPC]]) and reasoning chunks via [[src/renderer/src/screens/Chat/liveReasoningEvents.ts#upsertLiveReasoningChunk]]. Only genuinely new rows mount and play the spawn animation: the first chunk of a turn's answer, and the first chunk of a reasoning segment forced by a tool event (`forceNewSegment`).

Because the rows are never remounted mid-stream, their reveals can animate smoothly. The reasoning row uses a growth-driven typewriter ([[src/renderer/src/components/TypeAnimation.tsx#TypeAnimation]] with a `typing` state fed by TEXT GROWTH, not row position): typing starts on the first delta after mount, stays active while deltas arrive, and the reveal tail finishes smoothly after the last delta. The reasoning row ([[src/renderer/src/screens/Chat/HistoryRow.tsx#ReasoningRow]]) types at 100 chars/s; the answer bubble ([[src/renderer/src/screens/Chat/MessageRow.tsx#MessageRow]]) is NOT typed — it renders its full accumulated text instantly once the thought gate opens (faded in by the row's `messageIn` entrance), and late stream deltas paste in as they arrive. Typing a long streamed answer either strobes in chunky frames or is still mid-typewriter when a later thought row lands below, which reads as a stalled response; instant text plus the entrance fade avoids both (the caret is off for the same reason). The thought's reveal is hard-capped by a wall-clock budget (`maxDurationMs`: 900ms) — the per-tick reveal scales up so long text still finishes fast, but the budget is generous enough that the typewriter motion stays clearly visible instead of reading as a hard cut. The char cadence dominates for short text. A per-tick reveal ceiling (`maxCharsPerTick`: 18 for the thought, ~360 chars/s) bounds the duration-driven per-tick reveal — for very long text the budget alone would demand 100+ chars in one 50ms tick, a giant jump that reads as a blink; the ceiling keeps every tick a readable burst and the budget simply takes longer to empty. The answer's reveal is also gated behind its turn's thought: [[src/renderer/src/screens/Chat/MessageList.tsx#MessageList]] passes `waitForReasoningId` (the most recent reasoning row preceding each gated row) to the answer bubble AND every `ToolActivityGroup`, and [[src/renderer/src/screens/Chat/useReasoningGate.ts#useReasoningGate]] hides them (`.chat-message--hidden`) until the thought has both **stopped growing** and **fully revealed** ([[src/renderer/src/screens/Chat/reasoningStall.ts]] tracks each reasoning row's last growth timestamp AND its typewriter progress, reported by the reasoning row's render prop) — so the sequence reads "full thought → full response" instead of "partial thought → partial response" or both animating together. The gateway can deliver `message.complete` final text under several keys ("text"/"rendered", or RPC-style "final_response"/"output_text"/"content") — [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#applyDashboardStreamEvent]] reads them all; missing a shape made the final text empty so `completeAssistantWithFinalText` early-returned and NO answer bubble was created while `isLoading` still flipped false (chime fired, answer missing live — the intermittent report). The gateway can switch the conversation to a NEW runtime session mid-turn (auto-compact) while keeping the same stored session id — [[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#useDashboardChatTransport]] adopts such events (updating the runtime session id) instead of dropping them, so the live view keeps streaming instead of freezing until a session reopen. On the dashboard path the answer deltas are NOT rendered at all ([[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#useDashboardChatTransport]] passes `renderAssistantDeltas: false`, same as the remote/SSH path): the gateway emits answer text, then tools, then a trailing thought — rendering the deltas live puts a partial answer mid-transcript above the tools and trailing thought, which read as "cut" / "last response missing" no matter how the gate or merge was patched. Instead the thought streams live, and `message.complete` materializes the final answer ONCE, from the final text, at the end of the turn — the same shape as the (working) reopened-from-DB view. A quiet-finalize fallback ([[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#useDashboardChatTransport]]) covers the gateway occasionally completing a turn WITHOUT delivering `message.complete` (which would otherwise leave no answer at all — nothing streamed, nothing materialized): when the turn has been quiet for 10s while loading, the transport pulls the canonical rows from `state.db` via `getSessionMessages` and reconciles them in (`reconcileAfterDbRefresh`), finalizing only when the DB shows a completed answer for the last turn — otherwise it re-arms (a long-running tool just pauses). The fallback is guarded against firing into the NEXT turn: sending a new message resets the timer, and a reconcile only happens when `state.db` has caught up to the live transcript's last user message — a stale-DB reconcile would delete the just-sent message and resurrect the previous turn's canonical (fuller) answer. The merge + move-to-end logic in [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#completeAssistantWithFinalText]] remains for paths that do render deltas (legacy transport, mixed sessions). The FINAL safety net is the turn-end force release: when `isLoading` flips true→false in [[src/renderer/src/screens/Chat/Chat.tsx]] (the same moment the agent-finish chime plays), [[src/renderer/src/screens/Chat/reasoningStall.ts#forceReleaseAllReasoning]] makes every gate open unconditionally — whatever path ended the turn (message.complete, quiet-finalize, stall watchdog, abort), nothing stays hidden. [[src/renderer/src/screens/Chat/hooks/useChatActions.ts#handleSend]] resets the flag on the next send so the thought→response sequencing re-arms per turn. A safety net force-reveals after `REASONING_FORCE_REVEAL_MS` (5s) of thought quiet even when the typewriter never reported completion — the reasoning row may have been dropped by the end-of-stream reconciliation mid-reveal, or the turn's `message.complete` (which sets the settled marker AND flips `isLoading` false) may never arrive; without it the answer stays hidden until a full app restart clears the stale module-scope state. A tool / clarify boundary is a hard segment close (`reasoningSegmentClosed` in [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#applyDashboardStreamEvent]]): no more deltas will merge into the preceding thought, so [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#markTurnLastReasoningSettled]] marks that reasoning row settled via [[src/renderer/src/screens/Chat/reasoningStall.ts#markReasoningSettled]], and a settled row bypasses BOTH checks — [[src/renderer/src/screens/Chat/reasoningStall.ts#reasoningStalledMs]] returns MAX and [[src/renderer/src/screens/Chat/reasoningStall.ts#reasoningRevealComplete]] returns true — so the gate opens on the next 250ms poll instead of waiting the full `REASONING_SETTLE_MS` (1200ms) nor for the cosmetic typewriter to catch up. `message.complete` marks the trailing thought settled too (a turn ending on a trailing thought, or whose completion lands before `setIsLoading(false)` reaches the gate, would otherwise hide the already-streamed last answer — plus its file-changes badge — behind the gate until the thought's typewriter finished). The typewriter keeps animating cosmetically above while the answer reveals; resumed growth clears settled (alternating-tag streams that re-open the segment keep the gate honest). The whole accumulated answer does NOT type from 0 when the gate opens — it fades in whole via the row's `messageIn` entry animation (typing a long accumulated text from 0 would either strobe in 100+ char frames or take many seconds, both reading as blinks). Deltas that arrive AFTER the gate open simply paste into the full text (no typewriter, no snap point) so a later thought row landing below never finds the response mid-typewriter. An inactive row always renders its full text, so history and one-chunk-at-completion summaries paste in immediately.
