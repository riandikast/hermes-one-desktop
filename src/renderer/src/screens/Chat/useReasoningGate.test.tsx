...
function Gate({
  ...
  const { waiting } = useReasoningGate({ waitForReasoningId, hasContent });
...
describe("useReasoningGate", () => {
  it("shows immediately when there is no preceding reasoning to wait for", () => {
    const { getByTestId } = render(<Gate hasContent />);
    expect(getByTestId("row").textContent).toBe("shown");
  });

  it("shows when there is no content to gate", () => {
    const { getByTestId } = render(
      <Gate waitForReasoningId="reasoning-empty" hasContent={false} />,
    );
    expect(getByTestId("row").textContent).toBe("shown");
  });
  ...
  it("hides until the preceding thought stops growing and fully reveals, then shows", () => {
    ...
    const { getByTestId } = render(
      <Gate waitForReasoningId="reasoning-active" hasContent />,
    );
  ...
  it("never re-hides once the gate has opened", () => {
    ...
    const { getByTestId, rerender } = render(
      <Gate waitForReasoningId="reasoning-once" hasContent />,
    );
    expect(getByTestId("row").textContent).toBe("shown");
    ...
    rerender(<Gate waitForReasoningId="reasoning-once" hasContent />);
    act(() => {
