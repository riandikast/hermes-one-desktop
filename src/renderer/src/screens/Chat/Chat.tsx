...
function Chat({
  ...
  // virtual window's MessageListModel is now stale during the transition.

  // Chat.tsx keeps the ref shape so ChatNavArrow still compiles, but the real

  // pinned/follow behavior is `useStickToBottom.isAtBottom`.
  ...
  const chatDisplayModel = sessionModelOverride
    ? modelConfig.modelGroups
        .flatMap((group) => group.models)
        .find(
          (model) =>
            model.provider === sessionModelOverride.provider &&
            model.model === sessionModelOverride.model &&
            (model.baseUrl || "") === (sessionModelOverride.baseUrl || ""),
        )?.label ||
